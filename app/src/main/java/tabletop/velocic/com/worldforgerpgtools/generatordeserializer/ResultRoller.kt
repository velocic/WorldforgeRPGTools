...
class ResultRoller(
        ...
        for (i in 0 until numRolls) {
            ...
            for (entry in randomTable) {
                ...
                if (entryIsMatch) {
                    ...
                    val newItem = ResultItem(fullyQualifiedGeneratorPath, entry.name, entry.metadata.toMutableList())
    ...
    private fun determineTableDiceRange(tableEntries: List<TableEntry>): IntRange {
        var tableMinRoll = 1
        var tableMaxRoll = 1
